...
public class MoiMeetPlugin: NSObject, FlutterPlugin, FlutterStreamHandler {
    var flutterViewController: UIViewController
    var moiMeetViewController: MoiMeetViewController?
    ...
    public static func register(with registrar: FlutterPluginRegistrar) {
        ...
        let instance = MoiMeetPlugin(flutterViewController: flutterViewController)
    ...
    private func join(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        moiMeetViewController = MoiMeetViewController.init(options: options, eventSink: eventSink!)
        moiMeetViewController!.modalPresentationStyle = .overFullScreen
        flutterViewController.present(moiMeetViewController!, animated: true)
        result("Successfully joined meeting \(room)")
    }

    private func hangUp(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        moiMeetViewController?.jitsiMeetView?.hangUp()
    ...
    private func setAudioMuted(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        moiMeetViewController?.jitsiMeetView?.setAudioMuted(muted)
    ...
    private func setVideoMuted(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        let arguments = call.arguments as! [String: Any]
        let muted = arguments["muted"] as! Bool
        moiMeetViewController?.jitsiMeetView?.setVideoMuted(muted)
    ...
    private func sendEndpointTextMessage(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        moiMeetViewController?.jitsiMeetView?.sendEndpointTextMessage(message, to)
    ...
    private func toggleScreenShare(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        moiMeetViewController?.jitsiMeetView?.toggleScreenShare(enabled)
    ...
    private func openChat(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        let arguments = call.arguments as! [String: Any]
        let to = arguments["to"] as? String
        moiMeetViewController?.jitsiMeetView?.openChat(to)
    ...
    private func retrieveParticipantsInfo(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        moiMeetViewController?.jitsiMeetView?.retrieveParticipantsInfo({ (data:[Any]?) in self.eventSink!(["event": "participantsInfoRetrieved", "data": data])})
